...
export class FaturaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fatura = this.decodeTurkishCharacters(faturaData) as Fatura;
  }

  decodeTurkishCharacters(obj: any): any {
    if (typeof obj === 'string') {
      ...
      for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
          obj[key] = this.decodeTurkishCharacters(obj[key]);
        }
      }
    }
...
pdf.setLineWidth(4); 
    ...
    // Display Tablo 1 at the rightmost side
    const tablo1Data = [];
    ...
    pdf.autoTable({
      ...
      columnStyles: {
        ...
        1: { fontStyle: 'normal', fontSize: 7 } // İlgili sütunun stili
      },
      bodyStyles: { fontSize: 7, fontStyle: 'normal' }, // Hücre içindeki metin stilini belirtmek için
      headStyles: { fontStyle: 'bold', fontSize: 8 }, // Başlık satırındaki metin stilini belirtmek için
    });
...
const table2Data = [
  ['Sıra No', 'Stok Kodu', 'Mal Hizmet', 'Birim Fiyat', 'KDV Oranı', 'KDV Tutarı', 'Mal Hizmet Tutarı'],
...
pdf.autoTable({
  ...
  columnStyles: {
    ...
    1: { fontStyle: 'normal', fontSize: 7, halign: 'right',cellWidth: 100 } // Align the content in the rightmost column to the right
  },
...
// Move the cursor inside the rectangle
...
pdf.setFont('helvetica', 'bold'); // Kalın başlık için yazı tipini ve kalınlığı ayarla
pdf.setFont('helvetica', 'normal'); // Normal değer için yazı tipini ve kalınlığı ayarla
